...
const Packages = () => {
    ...
    useEffect(() => {
        fetch(`http://localhost:3020/api/packages/sort`)
            .then(respose => respose.json())
            .then(data => {
                setPackages(data.map(name => (
                    <div className="bg-header body">
                        <div key={name.id} className="col-sm-12 col-md-6 col-lg-4 temp white-box">
                            ...
                            <div><p className="purple">type:</p> <p>{name.sort}</p></div>
                        </div>
                    </div>
                )));
            });
            ...
            <div className="container body">
                <div className="row">
                    <div className="col-lg-12">
                        <h1 className="intro">Packages</h1>
                        <div>{packaged}</div>
                    </div>
                </div>
            </div>
